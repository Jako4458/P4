# EBNF
    ...
    Prog    -> Blocks Newline MCFunc Newline Blocks €
    ...
    Func    -> 'func' 'id' '(' Params ')' [-> Type] FuncBody
    ...
    FuncBody-> 'do' [Newline Stmnts] [Newline RetVal] Newline 'endfunc'

    RetVal  -> 'return' [Expr | 'id' | Literal]

    Stmnts  -> Stmnt [Newline Stmnts]
    ...
    Instan  -> Access 'id' ':' Type '=' ValExpr {',' 'id' ':' Type '=' ValExpr}
    ...
    LogicalExpr -> LogicalExpr 'or' OrExpr  
                |  OrExpr
    ...
    RelationExpr -> (NumExpr | id | Number_literal) RelationOp (NumExpr | id | Number_literal)
    ...
    For     -> 'for' Assign 'until' BoolExpr 'where' CompAssign 'do' [Newline Stmnts] Newline 'endfor'
    ...
    # unused 
    MCExpr  -> $<function already defined in mcfunction> Newline
    
    # unused
    Keyword -> 'if'       |  'elif'     |  'else'     |  'endif'    |  'until'
            |  'num'      |  'block'    |  'bool'     |  'string'   |  'in'
            |  'func'     |  'endfunc'  |  'return'   |  'foreach'
            |  'for'      |  'while'    |  'do'       |  'endfor'
