...
public class BuiltinFuncs {
    ...
    public static MSFile createSetB(String playerTag, String destIDName, String blockIDName, String relativeIDName) {
        ST start = new ST("summon minecraft:armor_stand ~ ~ ~ {Tags:[\"tpID\",\"setb\"],NoGravity:1} \n" +
        ...
        ST end = new ST("execute if score @p[tag=<playerTag>] <relativeID> matches 1 run execute as @e[tag=<blockID>, limit=1] at @s run clone ~ ~-1 ~ ~ ~-1 ~ 0 255 0\n" +
                "execute if score @p[tag=<playerTag>] <relativeID> matches 1 run execute as @e[tag=tpID, limit=1] at @s run clone 0 255 0 0 255 0 ~ ~ ~\n" +
                "execute if score @p[tag=<playerTag>] <relativeID> matches 0 run execute as @e[tag=<blockID>, limit=1] at @s run clone ~ ~-1 ~ ~ ~-1 ~ 0 255 0\n" +
                "execute if score @p[tag=<playerTag>] <relativeID> matches 0 run execute as @e[tag=tpID, limit=1] at @s run clone 0 255 0 0 255 0 ~ ~ ~\n" +
                "kill @e[tag=setb]\n");
